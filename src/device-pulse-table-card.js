...
class DevicePulseTableCard extends LitElement {
    ...
    constructor() {
        ...
        this._valueChangedCells = new Map();
        this._statusChangedRows = new Map();
    }
        ...
        if (this._filterText) {
            ...
            devices = devices.filter(
                (d) =>
                    ...
                    d.integration_name.toLowerCase().includes(filter)
            );
        }
    ...
    _renderCellValue(device, column) {
        ...
        if (column === "ping_status_since_timestamp" && device[column]) {
            const now = new Date();
            const diff = now - (new Date(device[column] * 1000));
            ...
            <ha-card>
                <div class="card">
                    ...
                    <div class="table-container">
                        ${devices.length === 0 ? html` <div class="no-data">No Device Found</div>` : 
                                            ...
                                            (device) => html`
                                                <tr class="device-status-${device['ping_status']['state']} ${this._statusChangedRows.has(device["device_id"]) ? 'status-changed' : ''}">
                                                    ${visibleColumns.map(
                                                        (col) => {
                                                            const valueChanged = 
                                                                    this._valueChangedCells.get(device.device_id)?.has(col) ||
                                                                    (this._valueChangedCells.get(device.device_id)?.size && col === "ping_status");
...
class DevicePulseTableCardEditor extends LitElement {
    ...
    _computeLabel(schema) {
        switch (schema.name) {
            ...
            case "show_status": return "Statues to Show";
            case "group_by_integration": return "Group-By Integration";
